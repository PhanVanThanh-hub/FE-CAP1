...
export const PostOwner = ({ postData }: { postData: any }) => {
  const nameUser = () => {
    if (postData) {
      return <div>{postData.author && <Text>{postData.author}</Text>}</div>;
    }
  };
...
export const Content = ({ postData }: { postData: any }) => {
    ...
    if (postData) {
      return (
        <div>
          ...
          {postData.image && <img src={postData?.image} alt="" width="450px" />}
        </div>
      );
    }
  };
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/get-comment/${postData.id}/`)
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/like/${postData.id}/`)
  ...
  const sumLike = () => {
    ...
    sum--;
    if (sum >= 1) return "and " + sum + " others";
  };
  return (
...
export const Comment = ({ comment }: { comment: any }) => {
  ...
  const viewComment = () => {
    if (comment) {
      return <div>{comment.content && <Text>{comment.content}</Text>}</div>;
    }
  };
  return (
    ...
    <Col>
      ...
      >
        <Row onClick={() => setBoolBag({ liked: !liked })}>
          ...
          <Row onClick={() => PutLike()}>
            <Text value={"1"} onClick={() => setLike("1")}>
              Like
            </Text>
          </Row>
        </Row>
